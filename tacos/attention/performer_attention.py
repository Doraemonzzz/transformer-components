...
class PerformerAttention(nn.Module):
    ...
    def feature_map(self, x):
        ...
        # exp(w^Tx - norm - c)
        feature = scale * torch.exp(x - norm.to(x) - torch.amax(x, dim=-1, keepdim=True).to(x).detach())
    ...
    # use only for test
    def feature_map_test(self, x):
        # input shape: ..., n, d
        scale = x.shape[-1] ** -0.5
        # ||x|| ^ 2 / 2
        norm = torch.sum(x ** 2, dim=-1, keepdim=True) / 2
        # exp(w^Tx - norm - c)
        feature = scale * torch.exp(x - norm.to(x))
    ...
    def test(self, x):
        ...
        # feature map
        q = self.feature_map_test(q_proj)
        k = self.feature_map_test(k_proj)
